...
func GetClient(configType string) kubernetes.Interface {
	var config *rest.Config

	switch configType {
	case "out":
		config = GetConfigOutOfCluster()
	case "in":
		fallthrough
	default:
...
// - ENV
// - $HOME/.kube/config
func GetConfigOutOfCluster() *rest.Config {
	kubeconfigPath := os.Getenv("KUBE_CONFIG_PATH")
	if kubeconfigPath == "" {
		log.Fatal("Missing 'KUBE_CONFIG' environment variable for config type 'out'")
	}
